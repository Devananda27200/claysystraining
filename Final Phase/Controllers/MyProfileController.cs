...
namespace E_ShoppingPortal.Controllers
{
    public class MyProfileController : Controller
    {
        ...
        public ActionResult Index(string editRequest, string firstname, string lastname, string password, string address, string bankdetails)
        {
            ...
            q1.CommandText = "UPDATE [Register] SET password='" + password + "',firstname='" + firstname + "',lastname='" + lastname + "',address='" + address + "',bankdetails='" + bankdetails + "' WHERE id='" + luser.id + "'";
            q1.ExecuteNonQuery();
            SqlCommand q = con.CreateCommand();
            q.CommandType = CommandType.Text;
            q.CommandText = "SELECT TOP 1 * FROM [Register] WHERE id='" + luser.id + "'";
